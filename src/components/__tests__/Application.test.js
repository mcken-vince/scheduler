...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));
    const input = getByPlaceholderText(appointment, "Enter Student Name");
    fireEvent.change(input, { target: { value: "Bobby McDylan" } });
    fireEvent.click(getByText(appointment, "Save"));

    expect(getByText(appointment, "Saving")).toBeInTheDocument();

    await waitForElement(() => getByText(appointment, "Bobby McDylan"));
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    expect(getByText(getAllByTestId(container, "day").find(day => queryByText(day, "Monday")), "1 spot remaining")).toBeInTheDocument();
  });
